...
1.【贏家分點動向】列出畫面中屬於「贏家」（近期波段獲利最多）的分點，目前是買超還是
   ...
   如果畫面裡的分點買賣超長條圖，可以跟KD/MACD交叉的時間點對照，請額外用一句話
   描述這個分點的買賣超「相對技術指標交叉時間點」大致是領先（買賣超先出現、技術
   指標之後才翻轉）、同步（幾乎同時發生）、還是落後（技術指標先翻轉、買賣超才
   跟上）。這只是根據畫面上少數幾波買賣超形狀做的粗略視覺觀察，不是逐日精確比對，
   也不是驗證過的規則，請用「大致」「看起來」這類保留字眼描述，不要講得像已經證實
   的固定行為模式；如果畫面模糊、買賣超波動次數太少、或兩個面板時間軸對不齊，直接
