...
def compute_institutional_signal(inst_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def signal_label(row):
        ...
        # 2026-09-04修正（法人訊號 vs 籌碼矛盾互相打架）：
        # n（買超法人數）只計算「外資/投信/自營」三類裡有幾類個別淨買超，跟total
        # （三大合計淨額）是兩個不同的東西——常見情況是兩類各自小額淨買超（n=2），
        # 但第三類（尤其自營商避險部位常見大額調節）當天大幅淨賣，讓三大合計實際上
        # 是負的。原本這裡完全沒檢查total正負號，只要n>=2就直接標「雙向買超/外資
        # 主導/三大齊買」這種聽起來像整體買超的字眼——這會跟同樣拿total判斷方向的
        # 「籌碼矛盾」欄位（margin_fetcher.compute_chip_conflict()）互相矛盾：法人
        # 訊號寫「買超」，籌碼矛盾卻寫「法人卻在賣」，兩欄對不上，使用者看了會以為
        # 系統資料兜不起來（2026-09-04實測案例：緯穎法人訊號顯示「⚡雙向買超」，
        # 三大合計卻是-1474張，籌碼矛盾正確寫出「法人卻在賣」）。
        # 修法：n>=2但total<0時，改標「⚠️ 法人方向分歧」，明講「多數類別買超但合計
        # 仍賣超」，不再用聽起來像買超的字眼誤導；total>=0時維持原本字眼跟行為不變。
        if total < 0:
            return "⚠️ 法人方向分歧（多類買超但合計仍賣超）"
        return base

    df["法人訊號"] = df.apply(signal_label, axis=1)

    # 排序：三大齊買 > 外資+投信 > 其他「真買超」情境 > 方向分歧（n高但合計仍賣超，
    # 比純粹的法人賣超更值得留意，但畢竟合計是賣超，優先度排在所有真買超訊號之後）
    order = {"🔥 三大齊買":0,"⚡ 外資+投信":1,"⚡ 外資主導":2,
             "⚡ 雙向買超":3,"🌱 投信單買":4,"🌱 外資單買":5,
             "⚠️ 法人方向分歧（多類買超但合計仍賣超）":6,
             "➖ 混合":7,"🔻 法人賣超":8}
    df["排序"] = df["法人訊號"].map(order).fillna(9)
